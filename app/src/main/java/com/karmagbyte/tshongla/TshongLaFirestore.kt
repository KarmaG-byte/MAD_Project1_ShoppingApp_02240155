...
object TshongLaFirestore {
    ...
    fun listenToProducts(onChange: (List<Product>) -> Unit, onError: (String) -> Unit = {}): ListenerRegistration {
        return products.addSnapshotListener { snapshot, error ->
            ...
            val result = snapshot?.documents.orEmpty()
                .mapNotNull(::productFromDocument)
                .filter { it.sellerId != "demo" }
                .sortedBy { it.id }
            onChange(result)
        }
    }
    ...
    /** Kept for preview/backward compatibility. Runtime data comes only from seller-created documents. */
    ...
    fun createBooking(
        ...
        users.document(customerId).get()
            .addOnSuccessListener { userDoc ->
                val role = userDoc.getString("role") ?: ""
                val phoneVerified = userDoc.getBoolean("phoneVerified") ?: false
                if (role != "customer") {
                    onError("Only customer accounts can reserve products.")
                    return@addOnSuccessListener
                }
                if (!phoneVerified) {
                    onError("Verify your phone number before reserving a product.")
                    return@addOnSuccessListener
                }
                createVerifiedBooking(product, customerId, quantity, onSuccess, onError)
            }
            .addOnFailureListener { error ->
                onError(error.localizedMessage ?: "Unable to verify your customer account")
            }
    }

    private fun createVerifiedBooking(
        product: Product,
        customerId: String,
        quantity: Int,
        onSuccess: (Booking) -> Unit,
    ...
    fun updateBookingStatus(booking: Booking, newStatus: String, onSuccess: () -> Unit = {}, onError: (String) -> Unit = {}) {
        val allowed = setOf("Reserved", "Ready for pickup", "Collected", "Cancelled")
        if (newStatus !in allowed) {
            onError("Invalid reservation status")
            return
        }
        ...
        db.runTransaction { transaction ->
            ...
            if (!bookingSnapshot.exists()) {
                throw IllegalStateException("Reservation no longer exists")
            }
            val oldStatus = bookingSnapshot.getString("status") ?: booking.status
            if (oldStatus == "Collected" || oldStatus == "Cancelled") {
                throw IllegalStateException("Completed reservations cannot be changed")
            }
    ...
    private fun productFromDocument(document: com.google.firebase.firestore.DocumentSnapshot): Product? {
        ...
        return Product(
            id,
            document.getString("name") ?: "Untitled product",
            category,
            document.getLong("price")?.toInt() ?: 0,
            document.getDouble("rating") ?: 0.0,
            document.getString("seller") ?: "Local seller",
            document.getString("sellerId") ?: "",
            document.getString("dzongkhag") ?: "Thimphu",
            document.getString("address") ?: "",
            document.getLong("stock")?.toInt() ?: 0,
            document.getLong("discountPercent")?.toInt() ?: 0,
            document.getString("symbol") ?: symbolForCategory(category),
    ...
    private fun bookingFromDocument(document: com.google.firebase.firestore.DocumentSnapshot): Booking? {
        ...
        val product = Product(
            productId,
            document.getString("productName") ?: "Reserved product",
            category,
            document.getLong("unitPrice")?.toInt() ?: 0,
            0.0,
            document.getString("seller") ?: "Local seller",
            sellerId,
            document.getString("dzongkhag") ?: "Thimphu",
        ...
        return Booking(
            ...
            document.getString("pickupCode") ?: document.id,
